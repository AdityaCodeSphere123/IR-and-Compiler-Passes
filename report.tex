...
After \texttt{mem2reg} the loop index is the only phi in \texttt{f2}.
\texttt{loopcarried} is a global, so it stays as \texttt{load}/\texttt{store}
and does not become a phi.

\begin{tabular}{llll}
\hline
Phi & Block & Incoming edge & Value \\
\hline
\texttt{\%.0} (loop index) & \texttt{\%1} & \texttt{\%0} $\to$ \texttt{\%1} (first time) & \texttt{0} \\
\texttt{\%.0} (loop index) & \texttt{\%1} & \texttt{\%17} $\to$ \texttt{\%1} (latch) & \texttt{\%18} ($i{+}1$) \\
\hline
\end{tabular}

The if/else that does \texttt{break} just jumps to the exit block. Those
edges do not feed a phi, because they do not produce a new SSA value.
...
At \texttt{-O0} most of the IR is stack slots: lots of \texttt{alloca},
\texttt{load} and \texttt{store} for locals like the loop index. After
\texttt{-O1} those temps are gone. The stores into \texttt{arr} in
\texttt{f1} are still there: \texttt{arr} is a local, but it is an
aggregate reached through GEPs, so \texttt{mem2reg} will not promote
it. That is also why \texttt{sroa} exists later.

The table was rebuilt from the current \texttt{opcodes.c} through
\texttt{script.py}. \texttt{f3} at \texttt{-O0} has five allocas
(\texttt{a}, \texttt{t}, the inner \texttt{long long a}, \texttt{b},
\texttt{d}) and case 3 shows up as \texttt{sitofp}, \texttt{fmul} and
\texttt{fptosi}. Empty \texttt{main} on this clang is just
\texttt{ret i32 0}, so it has no return-slot \texttt{alloca}.

The table also does not only shrink. \texttt{f2} has more blocks at
\texttt{-O2} than at \texttt{-O1}, so a higher level can make more IR,
not less.
...
The dump also contains wrappers like \texttt{function(...)} and
\texttt{loop(...)}, plus analyses and remarks
(\texttt{require}, \texttt{invalidate}, \texttt{recompute-globalsaa},
\texttt{verify}, \texttt{cg-profile}). Those were not counted. One
AArch64 class name appeared in the dump; it cannot go in
\texttt{-passes='...'} so it was skipped. Repeated transforms were
...
\texttt{instcombine} and \texttt{simplifycfg} stay at 8 times from
O1 up. They are re-run whenever an earlier pass may have created
new fold opportunities. The count does not grow; the slot numbers
do, because extra O2/O3 passes are inserted between those runs.

The same extra passes also push most other O1 names to a later
index. The opening five names
(\texttt{annotation2metadata} \ldots{} \texttt{lower-expect}) stay
in slots 1--5. After that, almost every index moves. A few
examples from our ordered lists:
...
\texttt{instcombine} is the same idea, but it has eight slots at
every level from O1, so they are easier to read on their own:
...
\texttt{memcpyopt} is the clearest move: at O1 it sits right after
the first sroa. At O2 it is delayed until after adce, because gvn
and jump-threading run first.

The dump can also change parameters when the name is the same.

\begin{lstlisting}[language={}]
loop-unroll, loop-vectorize, simple-loop-unswitch
\end{lstlisting}

\texttt{loop-unroll} is \texttt{loop-unroll<O1>}, \texttt{<O2>},
\texttt{<O3>}, so it is more willing to unroll at higher levels.
\texttt{loop-vectorize} at O1 is \texttt{vectorize-forced-only}; at
O2/O3 it is not, so real vectorisation is an O2 thing.
\texttt{simple-loop-unswitch} at O3 is
\texttt{<nontrivial;trivial>} and at O1/O2 it is only trivial
unswitch.

\section{Part D}

\texttt{f1}, \texttt{f2} and \texttt{f3} are the three required loops. \texttt{f4} is a small extra interleave so that
...
void f1(int n, float* restrict c, float* restrict a, float* restrict b, float alph) {
  for (int i = 0; i < n; i++)
    c[i] = a[i] * alph + b[i];
}

int f2(int n, int* restrict a, int* restrict b) {
  int dot = 0;
  for (int i = 0; i < n; i++)
    dot += a[i] * b[i];
  return dot;
...
void f3(int n, int* restrict c, int* restrict a, int* restrict b, int* restrict guess) {
  for (int i = 0; i < n; i++) {
    int a_val = a[i];
    int b_val = b[i];
...
Scalar IR first, then native vector IR on this Mac
(\texttt{arm64}, Homebrew LLVM 18.1.8). \texttt{-march=native} here
means NEON, not AVX.
...
\texttt{-Rpass-missed=loop-vectorize} did not print a miss. Clang still
touched the loop. The remarks that did print were:
...
The checksum casts each float to \texttt{int} before hashing, so a
last-bit FMA difference would not show up. That is expected:
contraction can change the last bits without meaning the kernels
disagree.
