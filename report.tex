...
it is promoted to a register. After the compare, the branch goes into the
loop body if the counter is still less than 3, and leaves the loop otherwise.
The increment adds one to that phi value and jumps back to the header, with
no store back to the stack.
...
in \texttt{vector.c}. \texttt{f4} is an extra interleave so that
\texttt{shufflevector} also appears as a store shuffle, not only as the
\texttt{alph} broadcast. \texttt{f5} returns the last value written in
the loop, so the vectoriser has to pull one lane out with
\texttt{extractelement}. Length is an argument. Array pointers
...
\begin{lstlisting}[language={}]
vector.c:9:  vectorized loop (vectorization width: 4, interleaved count: 4)
vector.c:17: vectorized loop (vectorization width: 4, interleaved count: 4)
vector.c:25: vectorized loop (vectorization width: 4, interleaved count: 4)
vector.c:34: vectorized loop (vectorization width: 4, interleaved count: 2)
vector.c:43: vectorized loop (vectorization width: 4, interleaved count: 4)
\end{lstlisting}

Those five lines are the \texttt{for} loops of \texttt{f1} through
\texttt{f5} in the submitted \texttt{vector.c}. \texttt{f1} starts at
line 9 because a one-line comment sits above the function. \texttt{f5}
was added at the end, so it did not move the earlier loops.
...
\texttt{f5} & $c[i]=3a[i]$, return last $c[i]$ & 4 & 4 \\
\hline
\end{tabular}
}
...
16 source iterations per trip of \texttt{f1}--\texttt{f3} and
\texttt{f5}. The leftover
...
\texttt{f5} reuses \texttt{<4 x i32>}; it does not add a new type.
...
The IR then chose one pointer and loaded through it. One pass of
that loop is:

\begin{lstlisting}
%14 = icmp eq i32 %13, 0
%15 = select i1 %14, ptr %3, ptr %2
%16 = getelementptr inbounds i32, ptr %15, i64 %11
%17 = load i32, ptr %16, align 4
\end{lstlisting}

\texttt{\%13} is \texttt{guess[i]}. If it is 0, \texttt{\%15} is the
\texttt{b} pointer; otherwise it is the \texttt{a} pointer. Then one
integer is loaded through that pointer and stored to \texttt{c[i]}.
There is no \texttt{<4 x i32> select}. An \texttt{if}/\texttt{else}
...
The mask is 0, 4, 1, 5, 2, 6, 3, 7. That order is
\texttt{a0}, \texttt{b0}, \texttt{a1}, \texttt{b1}, \texttt{a2},
\texttt{b2}, \texttt{a3}, \texttt{b3}. The
result is \texttt{<8 x i32>}, so eight ints go out in one store.
Interleave 2 means two of these shuffles per trip.

\textbf{\texttt{extractelement}.} \texttt{f2} never emits this. It
folds the four partial sums with
\texttt{llvm.vector.reduce.add.v4i32} instead of taking lanes out one
by one. \texttt{f4} did not change that; it only made another
shuffle. So \texttt{f5} stores \texttt{c[i] = 3 * a[i]} and
\texttt{return}s the last \texttt{c[i]}. That last integer has to
come out of the vector:

\begin{lstlisting}
%23 = mul nsw <4 x i32> %19, <i32 3, i32 3, i32 3, i32 3>
store <4 x i32> %23, ptr %27
%31 = extractelement <4 x i32> %23, i64 3
\end{lstlisting}

Each trip of \texttt{f5} does sixteen \texttt{i} values (width 4,
interleave 4). \texttt{\%23} is the last four. Lane 3 is the last of
those sixteen, so that is what the function returns when the whole
\texttt{n} fitted in the vector loop. For a leftover tail, the
scalar loop writes \texttt{last}, same as the C.
...
plus the integer arrays, the dot product, the last value returned
from \texttt{f5} and the \texttt{f5} stores. The two binaries
